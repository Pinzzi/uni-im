...
export default {
	...
	mutations: {
		updateDeviceAttempts(state, attempts) {
			state.deviceAttempts = attempts
		},
		updateDeviceLockouts(state, lockouts) {
			state.deviceLockouts = lockouts
		},
		clearDeviceAttempts(state, deviceId) {
			if (deviceId) {
				Vue.delete(state.deviceAttempts, deviceId)
			} else {
				state.deviceAttempts = {}
			}
		},
		clearDeviceLockouts(state, deviceId) {
			if (deviceId) {
				Vue.delete(state.deviceLockouts, deviceId)
			} else {
				state.deviceLockouts = {}
			}
		},
